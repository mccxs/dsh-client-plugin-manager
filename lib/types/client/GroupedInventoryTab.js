...
import css from './GroupedInventoryTab.module.css';
const PHASE_KEYS = {
    pending: 'pending',
...
function groupEntries(entries) {
    ...
    return PLUGIN_GROUP_ORDER.flatMap((key) => {
        ...
        return group === undefined ? [] : [{ key, entries: group }];
    });
}
/** Render the grouped, searchable current Loader inventory. */
export function GroupedInventoryTab({ list, t }) {
    ...
    return (_jsxs("div", { className: css.section, "aria-busy": state.status === 'loading', children: [state.status === 'loading' ? _jsx("p", { className: css.status, children: t('loading') }) : null, state.status === 'error' ? (_jsxs("div", { className: css.failure, children: [_jsx("p", { role: "alert", children: t('error') }), _jsx("button", { type: "button", onClick: retry, children: t('retry') })] })) : null, state.status === 'ready' ? (_jsxs("div", { className: css.catalog, children: [_jsxs("label", { className: css.search, children: [_jsx(IconSearchOutline16, { "aria-hidden": "true" }), _jsx("span", { className: css.visuallyHidden, children: t('search') }), _jsx("input", { type: "search", value: query, placeholder: t('search'), "aria-label": t('search'), onChange: (event) => { setQuery(event.currentTarget.value); } })] }), _jsxs("div", { className: css.catalogHeading, children: [_jsx("h3", { children: t('catalog') }), _jsx("span", { "data-plugin-count": filteredEntries.length, children: filteredEntries.length }), groups.length > 1 ? (_jsxs("span", { className: css.groupControls, children: [_jsx("button", { type: "button", onClick: expandAll, children: t('expandAll') }), _jsx("button", { type: "button", onClick: collapseAll, children: t('collapseAll') })] })) : null] }), state.snapshot.entries.length === 0 ? _jsx("p", { className: css.status, children: t('empty') }) : null, state.snapshot.entries.length > 0 && filteredEntries.length === 0
                        ? _jsx("p", { className: css.status, children: t('emptySearch') })
                        : null, groups.length > 0 ? (_jsx("div", { className: css.groups, children: groups.map((group) => {
                            const collapsed = collapsedGroups.has(group.key);
                            const groupLabel = t(PLUGIN_GROUP_LABEL_KEY[group.key]);
                            return (_jsxs("section", { className: css.group, "data-group": group.key, "data-collapsed": collapsed ? 'true' : undefined, children: [_jsxs("button", { className: css.groupToggle, type: "button", "aria-expanded": !collapsed, "aria-label": groupLabel, onClick: () => { toggleGroup(group.key); }, children: [_jsx(IconChevronDownOutline14, { className: css.groupChevron, size: 12, "aria-hidden": "true" }), _jsx("span", { className: css.groupName, children: groupLabel }), _jsx("span", { className: css.groupCount, "data-group-count": group.key, children: group.entries.length })] }), collapsed ? null : (_jsx("ul", { className: css.cards, children: group.entries.map((entry) => {
...
//# sourceMappingURL=GroupedInventoryTab.js.map